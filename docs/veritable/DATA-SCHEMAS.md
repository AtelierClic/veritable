...
Depuis le J4 (`schemaVersion: 4`) le contournement est un enregistrement par bien, et la sauvegarde porte les sections du moteur politique (voir la section « Politique » ci-dessous quand elle existe). Schéma de référence : `src/veritable/data/schemas/save.ts` ; v1, v2 et v3 figées dans `saveV1.ts`, `saveV2.ts` et `saveV3.ts`.
