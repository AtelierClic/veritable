...
- Livraison du J5 (100 campagnes × 50 ans sur le cœur, cinq processus) : `npx tsx tools/veritable/headless/delivery.ts --runs 100 --years 50 --parallel 5 --seed 1 --core --out <dossier> --report docs/veritable/reports/J5` ; `--aggregate` relit les campagnes déjà écrites dans `--out`. Rapport du J4 : `npx tsx tools/veritable/headless/reportJ4.ts --out docs/veritable/reports/J4`.
...
- Monde (J6) : carte `npm run veritable:borders -- calibrate --map giantworldmap`, puis `rasterize --scenario world-2026` et `zones --scenario world-2026` ; données `npm run veritable:ingest -- fetch-world --scenario world-2026` (réseau), `fetch-wikidata-world --scenario world-2026 [--only A,B]` (réseau), puis `build --scenario world-2026` et `build-leaders --scenario world-2026` (hors ligne).
- Tests nucléaires forcés : `npx tsx tools/veritable/headless/nuclearTests.ts --test invasion --seeds 20 --years 12 --divisions 80 --radius 20 --parallel 5 --out docs/veritable/reports/J6/nuclear` ; `--test dead-hand --seeds 20`.
- Performance sur le cœur (tick, sauvegardes, tas) : `node --expose-gc --import tsx tools/veritable/headless/perfWorld.ts --scenario world-2026 --years 50 --windows 2026,2040,2060,2075 --out <dossier>` ; les sauvegardes écrites se chargent dans le jeu (mesure ×5 du navigateur).
- Campagnes mondiales (J6c) : `npx tsx tools/veritable/headless/delivery.ts --runs 30 --years 50 --parallel 5 --seed 1 --scenario world-2026 --core --out <dossier> --report docs/veritable/reports/J6/world`.
- Tests joués (J6c, serveur de développement lancé, Microsoft Edge) : `npx tsx tools/veritable/playtest/guideJ4.ts` (idem `guideJ5.ts`, `guideJ6.ts`) ; captures et chiffres dans `docs/veritable/reports/J6/playtest/`. Ne rien écrire dans le dépôt pendant un test : Tailwind régénère la feuille de style et la page se recharge.
